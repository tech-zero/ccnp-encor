...
def get_saved_config_cisco(host, username, password, enable_secret):
    ...
    backupFile = open("backpu-config/" + fileName + ".txt", "w+")
...
def get_saved_config_juniper(host, username, password,):
    ...
    # Creates the connect to the device.
...
def get_saved_config_vyos(host, username, password,):
    ...
    # Gets and splits the hostname for the output file name.
    hostname = net_connect.send_command("show conf | get host-name | awk {'print $2'}")
    hostname = hostname.split()
    hostname = hostname[0]
    # Creates the file name, which is the hostname, and the date and time.
    fileName = hostname + "_" + dt_string
...
def get_saved_config_microtik(host, username, password,):
    ...
    # Gets the running configuration.
    output = net_connect.send_command("export", delay_factor=40)
    # Gets and splits the hostname for the output file name.
    hostname = net_connect.send_command("system identity print")
    hostname = hostname.split()
    hostname = hostname[1]
    # Creates the file name, which is the hostname, and the date and time.
    fileName = "config_backp-" + hostname + "_" + dt_string
    # Creates the text file in the backup-config folder with the special name, and writes to it.
    backupFile = open("backup-config/" + fileName + ".txt", "w+")
    backupFile.write(output)
    print("Outputted to " + fileName + ".txt")
...
def csv_option_cisco():
    ...
    with open(csv_name, 'r') as read_job:
        ...
        while rows >= 2:
           ...
           if ip_ping == None:
               ...
               downDeviceOutput = open("backpu-config/" + fileName, "a")
...
# Get the CSV file name for VyOS device, and grabs the information from it.
def csv_option_vyos():
...
# Get the CSV file name for Huawei device, and grabs the information from it.
...
# Get the CSV file name for MicroTik device, and grabs the information from it.
